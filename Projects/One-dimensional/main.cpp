...
int main()
{
	...
	//		string devArg2 = to_string(abs(a.GetMinArgShekel()[1] - ans[4]));
	...
	////вывод для одномерного варианта функций Шекеля и Хилла
	//cout << "  Shekel Problem Family" << std::endl;
	...
	//	for (int i = 0; i < 1000; i++)
	//	{
	//		GlobalSearch a(0, 10, 4, i);
	//		ans = a.startShekel();
	//		string devValue = to_string(abs(ans[1] - a.GetMinVShekel()));
	//		string devArg = to_string(abs(a.GetMinArgShekel() - ans[3]));
	//		devArg.replace(devArg.find("."), 1, ",");
	//		devValue.replace(devValue.find("."), 1, ",");
	//		//cout << a.GetMinVShekel() << "\t" << a.GetMinArgShekel();
	//		fout << i << ";" << ans[0] << ";" << devArg << ";" << devValue << ";" << ans[2] << endl;
	//	}
	//}
	//fout.close();

	//cout << "  Shekel Problem Family" << std::endl;
	//fout.open("ResultShekel.txt");
	//fout << "Function number; number of iterations; deviation from the global minimum by argument; Deviation from the global minimum in the value of the function; time\n";
	//if (fout.is_open())
	//{
	//	for (double r = 2; r <= 10; r += 0.01)
	...
	//	for (int i = 0; i < 1000; i++)
	//	{
	//		GlobalSearch a(0, 1, 7.49, i);
	//		ans = a.startHill();
	//		string devValue = to_string(abs(ans[1] - a.GetMinVHill()));
	//		string devArg = to_string(abs(a.GetMinArgHill() - ans[3]));
	//		devArg.replace(devArg.find("."), 1, ",");
	//		devValue.replace(devValue.find("."), 1, ",");
	//		//cout << a.GetMinVShekel() << "\t" << a.GetMinArgShekel();
	//		fout << i << ";" << ans[0] << ";" << devArg << ";" << devValue << ";" << ans[2] << endl;
	...
	//			GlobalSearch a(0, 10, r, i);
	//			ans = a.startHill();
	//			double devValue = abs(ans[1] - a.GetMinVHill());
	//			if (devValue > maximumDeviation)
	//				maximumDeviation = devValue;
	//			averageDeviation += devValue;
	//		}
