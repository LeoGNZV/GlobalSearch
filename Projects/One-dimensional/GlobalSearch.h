...
public:
	...
	vector<double> startShekel(); // алгоритм глобального поиска для функций семейства Шекеля

	vector<double> startHill(); // алгоритм глобального поиска для функций семейства Хилла
	...
	double GetMinArgShekel()
	{
		return shekelFam[index]->GetOptimumPoint()[0];
	}

	double GetMinArgHill()
	{
		return hillFam[index]->GetOptimumPoint()[0];
	}
}; 