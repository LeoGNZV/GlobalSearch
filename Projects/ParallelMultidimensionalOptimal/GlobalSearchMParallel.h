...
class GlobalSearchM
{
	...
	struct Compare {
		bool operator()(Interval left, Interval right) const {
			return left.getValueRt() < right.getValueRt(); // <????????????????????????????????
		}
	};
	std::priority_queue< Interval, std::vector<Interval>, Compare> testSequenceInterval;
